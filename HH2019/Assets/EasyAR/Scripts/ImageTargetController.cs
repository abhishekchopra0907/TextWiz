...
public class ImageTargetController : MonoBehaviour
{
    ...
    private IEnumerator LoadImageTarget()
    {
        ...
        ImageTracker.LoadImageTarget(this, (_target, status) =>
        {
            ...
            Debug.Log("[EasyAR] Target name: " + _target.name() + " target runtimeID: " + _target.runtimeID() + " load status: " + status);
    ...
    public void OnLost()
    {
        Debug.Log("[EasyAR] OnLost target name: " + target.name());
    ...
    public void OnFound()
    {
        ...
        window.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f);
        Debug.Log("[EasyAR] OnFound target name: " + target.name());
        gameObject.SetActive(true);
        window.transform.SetParent(window.GetComponent<parentSc>().parent);
